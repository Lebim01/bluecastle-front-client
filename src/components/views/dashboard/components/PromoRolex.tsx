...
const PromoRolex: FC<Props> = ({ i18n }) => {
    ...
    <Card isFooterBlurred className="w-full h-[300px]">
      <CardHeader className="absolute z-10 top-1 justify-between">
        <div className="flex flex-col items-start">
          ...
          <h4 className="text-gray-900 font-medium text-xl">ROLEX</h4>
        </div>
        <Image
      ...
      <CardFooter className="absolute bg-black/40 bottom-0 z-10 border-t-1 border-default-600 dark:border-default-100">
        <div className="flex flex-grow gap-2 items-center">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-tiny text-white/60 uppercase">{i18n.t('rank_view.ranks.diamond')} (150K)</p>
          </div>
        </div>
        {rank && rank.points && (
          <Progress
            ...
            value={
              ((rank?.points[rank?.points?.smaller] || 1) /
                (RanksObject().diamante.start_points +
                  RanksObject().diamante.points)) *
              100
            }
          />
        )}
